...
public class EntityAIFollowOwner extends EntityAIBase {
	...
	public EntityAIFollowOwner(EntityHeadPet pet, double followSpeedIn, float minDistIn, float maxDistIn)
	{
		this.pet = pet;
		...
		if (!(pet.getNavigator() instanceof PathNavigateGround) && !(pet.getNavigator() instanceof PathNavigateFlying))
		{
	...
	public boolean shouldExecute()
	{
		...
		else if (this.pet.getDistanceSq(entitylivingbase) < (double)(this.minDist * this.minDist))
		{
			return false;
		}
		else
	...
	{
		return !this.petPathfinder.noPath() && this.pet.getDistanceSq(this.owner) > (double)(this.maxDist * this.maxDist);
	}
	...
	{
		this.pet.getLookHelper().setLookPositionWithEntity(this.owner, 10.0F, (float)this.pet.getVerticalFaceSpeed());
		...
		if (--this.timeToRecalcPath <= 0)
		{
			...
			{
				if (!this.pet.getLeashed() && !this.pet.isRiding())
				{
					if (this.pet.getDistanceSq(this.owner) >= 144.0D)
					{
						...
						for (int l = 0; l <= 4; ++l)
						{
							for (int i1 = 0; i1 <= 4; ++i1)
							{
								...
								{
									this.pet.setLocationAndAngles((double)((float)(i + l) + 0.5F), (double)k, (double)((float)(j + i1) + 0.5F), this.pet.rotationYaw, this.pet.rotationPitch);
									this.petPathfinder.clearPath();
									return;
								}
							}
